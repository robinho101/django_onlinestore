  ...
  document.querySelector(selector1).addEventListener("click", (e) => {
    ...
    if (stName == "left") {
      ...
      let current = document
        ...
        .style[stName].replace(/\D/g, "");
      let res = -Number(current) - stValue;
      document.querySelectorAll(selector2).forEach((elem, index) => {
        if (
          document.querySelectorAll(".other-item-card")[
            document.querySelectorAll(".other-item-card").length - 1
          ].offsetLeft <= 1082
        ) {
      ...
      let res = Number(current) + stValue;
      document.querySelectorAll(selector2).forEach((elem, index) => {
        if (document.querySelectorAll(".other-item-card")[0].offsetLeft >= 0) {
          ...
          elem.style[stName] = -String(res) + "px";
          if (
            document.querySelectorAll(".other-item-card")[1].offsetLeft >= 0
          ) {
            document.querySelector(".right-arw").style["display"] = "None";
            return;
          }
...
document
  .querySelector(".search-submit-block")
  .addEventListener("click", async (e) => {
    ...
    let csrf_token = document.querySelector(".form_product_in_basket_to_model")[
      (name = "csrfmiddlewaretoken")
    ].value;

    for (let elem of document.querySelectorAll(".search-label-options")) {
      if (elem.selected) {
        selectedCategoryId = Number(
          elem.previousSibling.data.replace(/\D+/g, "")
        );
        break;
      }
    }
...
getAmountOfProduct(
  "http://127.0.0.1:8000/product-in-basket-to-model/",
  ".span-amount-of-product"
);
getAmountOfProduct(
  "http://127.0.0.1:8000/toUserSelectionModel/",
  ".span-amount-of-product_in_user_selection"
);
//////////
