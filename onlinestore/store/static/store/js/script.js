  ...
  document.querySelector(selector1).addEventListener("click", (e) => {
    ...
    if (stName == "left") {
      ...
      if(document.querySelectorAll('.other-item-card')[0].offsetLeft >= 0) {
        ...
        elem.style['left'] ='';
        elem.style[stName] =-String(res)+'px';
      if(document.querySelectorAll('.other-item-card')[1].offsetLeft >= 0) {
        document.querySelector('.right-arw').style['display'] = 'None';
        return;
      }
      }
...
//    Array.from(document.querySelectorAll('.search-label-options')).some((elem)=>{
//    if(elem.selected) {
...
//    });
    ...
    selectedCategoryId = selectedCategoryId==undefined ? selectedCategoryId='': selectedCategoryId;
